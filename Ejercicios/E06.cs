...
public class E06 : MonoBehaviour {
	...
	void Start () {
		...
		bool[] niveles = new bool[]{false,false,false};
	...
	*/
	...
	void Start () {
		...
		int[] valoresEnteros = new int[]{1,1,1,1,1};
		
		// 2b. Saca por log el numero de elementos que tiene el array:
		// Pista: .Length
		Debug.Log (valoresEnteros.Length);
		...
		Debug.Log(valoresEnteros[0]);
		Debug.Log(valoresEnteros[1]);
		Debug.Log(valoresEnteros[2]);
		Debug.Log(valoresEnteros[3]);
		Debug.Log(valoresEnteros[4]);
	...
	*/
	...
	void Start () {
		...
		string [] predicciones = {"ganaras la partida","comeras tortilla","ganaras su confianza","seras unico para ellos","te sera difisil ayudar"};
		...
		Debug.Log (predicciones[numeroAleatorio]);
