...
public class E14 : MonoBehaviour {
	...
	*/
	...
	void Start () {
		...
		foreach (int num in valores) {
			Debug.Log("Vuelta "+numeroElementoVuelta+": "+num);
	...
	*/
	...
	void Start () {
		...
		for (int cont1=0,cont2=9; cont1<textosNormales.Length; cont1++,cont2--) {
			textosInvertidos[cont1]= textosNormales[cont2];		
		}
		
		// 4d. Usando un bucle "foreach" recorre todos los elementos del vector (array)
		// "textosInvertidos" y sacalos por log:
		foreach (string elem in textosInvertidos) {
			Debug.Log(elem);		
		}
